...
private:
    ...
    public:
        Interrupt(
            ...
            void* context
        ) : _pin(pin), _affinity(affinity), _callback(callback), _context(context) {}
        ...
        System::PlatformResult Register(bool startEnabled) noexcept {
            const esp_err_t added = gpio_isr_handler_add(
                static_cast<gpio_num_t>(_pin), &Dispatch, this
            );
            if (added != ESP_OK) return PlatformResultFromEspError(added);
            _registered = true;
            if (startEnabled) return Enable();
            (void)gpio_intr_disable(static_cast<gpio_num_t>(_pin));
    ...
    static System::PlatformResult EnsureInterruptService(System::ProcessorAffinity affinity) {
...
public:
    ...
    System::PlatformResult Write(System::GPIO::Pin pin, System::GPIO::State state) noexcept override {
        const auto nativePin = static_cast<gpio_num_t>(pin);
        if (!GPIO_IS_VALID_OUTPUT_GPIO(nativePin)) {
            return System::PlatformResult::Failed(System::PlatformStatus::InvalidArgument);
        }
    ...
    System::PlatformResult Read(System::GPIO::Pin pin, System::GPIO::State& state) const noexcept override {
        const auto nativePin = static_cast<gpio_num_t>(pin);
        if (!GPIO_IS_VALID_GPIO(nativePin)) {
            return System::PlatformResult::Failed(System::PlatformStatus::InvalidArgument);
        }
        state = gpio_get_level(nativePin) != 0 ? System::GPIO::State::High : System::GPIO::State::Low;
        return System::PlatformResult::Succeeded();
    }
    ...
    System::GPIO::InterruptCreationResult CreateInterrupt(
        ...
        auto interrupt = std::make_unique<Interrupt>(pin, configuration.Affinity, callback, context);
...
inline IDFGPIOController& IDFGPIO() noexcept {
    static IDFGPIOController controller;
    return controller;
}

inline void InstallGPIOController() noexcept {
    System::GPIO::SetController(&IDFGPIO());
}
