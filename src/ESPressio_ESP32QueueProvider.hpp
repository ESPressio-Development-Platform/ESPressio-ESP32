...
class ESP32MessageQueue final : public System::Queue::IMessageQueue {
private:
...
public:
    ESP32MessageQueue(std::size_t elementSize, std::size_t capacity)
    ...
    std::size_t Size() const noexcept override {
        return _queue != nullptr
            ? static_cast<std::size_t>(uxQueueMessagesWaiting(_queue))
            : 0;
    }
};

class ESP32QueueProvider final : public System::Queue::IQueueProvider {
public:
    std::unique_ptr<System::Queue::IMessageQueue> Create(
        std::size_t elementSize,
        std::size_t capacity
    ) override {
        auto queue = std::make_unique<ESP32MessageQueue>(elementSize, capacity);
        if (!queue->IsAvailable()) return nullptr;
        return queue;
    }
};

inline ESP32QueueProvider& QueueProvider() noexcept {
    static ESP32QueueProvider provider;
    return provider;
}

inline void InstallQueueProvider() noexcept {
    System::Queue::SetProvider(&QueueProvider());
}
