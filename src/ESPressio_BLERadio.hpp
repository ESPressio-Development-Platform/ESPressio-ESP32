...
private:
    ...
    static constexpr std::size_t ManufacturerPrefixBytes = 3; // company identifier + frame marker
    static constexpr std::size_t LinkEnvelopeBytes = ManufacturerPrefixBytes + AddressBytes;
    static constexpr std::size_t AdvertisingStructureBytes = 2; // AD length + AD type
    static constexpr std::size_t MaximumPayloadBytes =
        AdvertisementBytesMaximum - AdvertisingStructureBytes - LinkEnvelopeBytes;
    ...
    static BLERadio*& CallbackInstance() noexcept {
        static BLERadio* instance = nullptr;
        return instance;
    }

    static bool IsBroadcastAddress(const uint8_t* address) noexcept {
        if (address == nullptr) return false;
        for (std::size_t i = 0; i < AddressBytes; ++i) {
    ...
    void ReleaseOwnedBluetoothStack() noexcept {
        if (_bluedroidEnabledByUs && esp_bluedroid_get_status() == ESP_BLUEDROID_STATUS_ENABLED) {
            (void)esp_bluedroid_disable();
        }
        if (_bluedroidInitializedByUs && esp_bluedroid_get_status() == ESP_BLUEDROID_STATUS_INITIALIZED) {
            (void)esp_bluedroid_deinit();
        }
        if (_controllerEnabledByUs && esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_ENABLED) {
            (void)esp_bt_controller_disable();
        }
        if (_controllerInitializedByUs && esp_bt_controller_get_status() == ESP_BT_CONTROLLER_STATUS_INITED) {
    ...
    bool EnqueueTransmit(
        ...
        const uint8_t next = static_cast<uint8_t>(
            (_transmitWriteIndex + 1u) % ESPRESSIO_ESP32_BLE_RADIO_TX_QUEUE_DEPTH
        );
    ...
    bool DequeueTransmit(TransmitPacket& packet) noexcept {
        ...
        if (_transmitReadIndex != _transmitWriteIndex) {
            ...
            _transmitReadIndex = static_cast<uint8_t>(
                (_transmitReadIndex + 1u) % ESPRESSIO_ESP32_BLE_RADIO_TX_QUEUE_DEPTH
            );
            available = true;
        }
        portEXIT_CRITICAL(&_transmitMux);
    ...
    void BuildAdvertisement(const TransmitPacket& packet) noexcept {
        ...
        _activeAdvertisement[0] = static_cast<uint8_t>(1u + manufacturerDataBytes); // type + manufacturer data
        ...
        _activeAdvertisement[3] = static_cast<uint8_t>(
            (_configuration.ManufacturerCompanyIdentifier >> 8u) & 0xFFu
        );
        _activeAdvertisement[4] = FrameMarker;
        std::memcpy(_activeAdvertisement.data() + 5, packet.Destination.Bytes.data(), AddressBytes);
        if (packet.Length != 0) {
            std::memcpy(_activeAdvertisement.data() + 5 + AddressBytes, packet.Payload.data(), packet.Length);
        }
    }
    ...
    void ArmTransmitDwell() noexcept {
        ...
        (void)esp_timer_stop(_transmitTimer);
        const uint64_t dwellMicroseconds =
            static_cast<uint64_t>(_configuration.TransmissionDwellMilliseconds) * 1000ULL;
        if (esp_timer_start_once(_transmitTimer, dwellMicroseconds) != ESP_OK) {
    ...
    void AdvanceTransmitQueue() noexcept {
        ...
        while (DequeueTransmit(packet)) {
            ...
            const esp_err_t result = esp_ble_gap_config_adv_data_raw(
                _activeAdvertisement.data(),
                static_cast<uint32_t>(ActiveAdvertisementLength(packet))
            );
        ...
        // Close the race where a sender queued work while the previous cycle was being retired.
        bool expected = false;
        portENTER_CRITICAL(&_transmitMux);
        ...
        if (
            hasQueuedWork &&
            _transmitCycleActive.compare_exchange_strong(expected, true, std::memory_order_acq_rel)
        ) {
            AdvanceTransmitQueue();
        }
    }
    ...
    void QueueReceivedAdvertisement(const esp_ble_gap_cb_param_t::ble_scan_result_evt_param& scan) noexcept {
        ...
        while (offset < total) {
            ...
            if (data[offset + 1u] == ManufacturerSpecificType) {
                ...
                const uint16_t companyIdentifier = static_cast<uint16_t>(manufacturer[0]) |
                    (static_cast<uint16_t>(manufacturer[1]) << 8u);
                if (
                    companyIdentifier == _configuration.ManufacturerCompanyIdentifier &&
                    manufacturer[2] == FrameMarker
                ) {
                    const uint8_t* destination = manufacturer + ManufacturerPrefixBytes;
                    if (
                        std::memcmp(destination, _localAddress.Bytes.data(), AddressBytes) != 0 &&
                        !IsBroadcastAddress(destination)
                    ) return;
                    ...
                    const uint8_t next = static_cast<uint8_t>(
                        (write + 1u) % ESPRESSIO_ESP32_BLE_RADIO_RX_QUEUE_DEPTH
                    );
                    ...
                    packet.Destination = Radio::RadioAddress::FromBytes(
                        destination,
                        static_cast<uint8_t>(AddressBytes)
                    );
                    packet.Length = static_cast<uint8_t>(payloadSize);
                    ...
                    if (payloadSize != 0) {
                        std::memcpy(
                            packet.Payload.data(),
                            manufacturer + ManufacturerPrefixBytes + AddressBytes,
                            payloadSize
                        );
                    }
        ...
        switch (event) {
            case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT:
                if (
                    ...
                    parameter.scan_param_cmpl.status == ESP_BT_STATUS_SUCCESS &&
                    esp_ble_gap_start_scanning(0) == ESP_OK
                ) {
                    // The definitive active state arrives in SCAN_START_COMPLETE.
                }
                break;

            case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
                _scanActive.store(
                    parameter.scan_start_cmpl.status == ESP_BT_STATUS_SUCCESS,
            ...
            case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT:
                if (parameter.adv_data_raw_cmpl.status != ESP_BT_STATUS_SUCCESS) {
                    AdvanceTransmitQueue();
                    break;
                }
                if (_advertisingActive.load(std::memory_order_acquire)) {
                    ArmTransmitDwell();
                } else if (esp_ble_gap_start_advertising(&_advertisingParameters) != ESP_OK) {
                    AdvanceTransmitQueue();
                }
                break;
...
public:
    ...
    explicit BLERadio(BLERadioConfiguration configuration = {}) noexcept
    ...
    bool Start() override {
        ...
        if (
            _configuration.ScanWindow == 0 ||
            _configuration.ScanInterval == 0 ||
            _configuration.ScanWindow > _configuration.ScanInterval ||
            _configuration.TransmissionDwellMilliseconds == 0
        ) return false;

        if (!ResolveLocalAddress()) return false;
        if (!EnsureBluetoothStack()) {
            ReleaseOwnedBluetoothStack();
            return false;
        }
        if (!CreateTransmitTimer()) {
            ReleaseOwnedBluetoothStack();
            return false;
        }
    ...
    /// <summary>Returns whether this provider has accepted its BLE start sequence.</summary>
    bool IsStarted() const noexcept override { return _started.load(std::memory_order_acquire); }

    /// <summary>Reports BLE legacy-advertising facilities exposed to ESPressio-Radio.</summary>
    ...
    /// <summary>Returns the ESP32 public Bluetooth device address used as this Radio link address.</summary>
    Radio::RadioAddress LocalAddress() const noexcept override { return _localAddress; }

    /// <summary>
    /// Queues one bounded opaque Radio packet for repeated legacy BLE advertising before the next queued packet is sent.
    /// </summary>
    Radio::RadioSendResult Send(
        ...
        if (!destination.IsValid() || destination.Length != AddressBytes)
        ...
        if (!EnqueueTransmit(destination, payload, payloadSize))
            return complete({Radio::RadioSendStatus::Busy, 0});

        bool expected = false;
        if (_transmitCycleActive.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
            AdvanceTransmitQueue();
        }
        return complete(Radio::RadioSendResult::Accepted());
    }

    /// <summary>Installs the RadioWorker-owned packet receiver.</summary>
    void SetReceiver(Radio::IRadioReceiver* receiver) noexcept override { _receiver = receiver; }

    /// <summary>Installs the task-context RadioWorker wake signal used by BLE GAP receive callbacks.</summary>
    void SetWorkSignal(Radio::IRadioWorkSignal* signal) noexcept override {
        _workSignal.store(signal, std::memory_order_release);
    }

    /// <summary>Gets the shared ESPressio Radio observable subscription surface.</summary>
    Radio::RadioObserverSubscriptions& Observers() noexcept override { return _observers; }

    /// <summary>Drains bounded BLE receive storage into RadioWorker-owned processing context.</summary>
    void DrainInbound() override {
        while (true) {
            ...
            view.Flags = packet.Destination.IsBroadcast()
            ...
            _receiveReadIndex.store(
                static_cast<uint8_t>((read + 1u) % ESPRESSIO_ESP32_BLE_RADIO_RX_QUEUE_DEPTH),
                std::memory_order_release
            );
        }
    }
