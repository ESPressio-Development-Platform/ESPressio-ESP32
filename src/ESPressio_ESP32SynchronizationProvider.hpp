...
class ESP32BinarySignal final : public System::Synchronization::ISignal {
private:
    SemaphoreHandle_t _semaphore = nullptr;

public:
    explicit ESP32BinarySignal(bool initiallySet) {
        _semaphore = xSemaphoreCreateBinary();
        if (_semaphore != nullptr && initiallySet) {
            xSemaphoreGive(_semaphore);
        }
    }

    ~ESP32BinarySignal() override {
    ...
    System::PlatformResult Wait(uint32_t timeoutMilliseconds) noexcept override {
        ...
        if (timeoutMilliseconds != System::Synchronization::WaitForever) {
            ...
            if (timeoutMilliseconds != 0 && ticks == 0) {
                ticks = 1;
            }
        }
...
class ESP32SynchronizationProvider final
    : public System::Synchronization::ISynchronizationProvider {
public:
    std::unique_ptr<System::Synchronization::ISignal> CreateBinarySignal(
        bool initiallySet = false
    ) override {
        auto signal = std::make_unique<ESP32BinarySignal>(initiallySet);
...
inline ESP32SynchronizationProvider& SynchronizationProvider() noexcept {
    static ESP32SynchronizationProvider provider;
    return provider;
}

inline void InstallSynchronizationProvider() noexcept {
    System::Synchronization::SetProvider(&SynchronizationProvider());
}
